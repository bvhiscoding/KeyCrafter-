...
const ProductForm = () => {
  ...
  const raw_cat  = categoriesData?.data?.items ?? categoriesData?.data ?? categoriesData?.categories;
  const categories = Array.isArray(raw_cat) ? raw_cat : [];

  const raw_brand = brandsData?.data?.items ?? brandsData?.data ?? brandsData?.brands;
  const brands    = Array.isArray(raw_brand) ? raw_brand : [];

  const raw_prod  = productsData?.data?.items ?? productsData?.data ?? productsData?.products;
          ...
          >
            ...
            >
              {preview ? (
                ...
                <div
                  style={{ textAlign: "center", color: "rgba(0,245,255,0.3)" }}
                >
                  <UploadIcon />
                  <p style={{ fontSize: "0.65rem", marginTop: "0.3rem" }}>
                    No image
                  </p>
                </div>
              )}
